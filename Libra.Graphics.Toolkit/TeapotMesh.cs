...
{
    public sealed class TeapotMesh : BezierMesh
    {
        public TeapotMesh(DeviceContext context, float size = 1.0f, int tessellation = 8)
            : base(context)
        {
            ...
            int vertexCountPerPatch = CalculateVertexCountPerPatch(tessellation);
            int indexCountPerPatch = CalculateIndexCountPerPatch(tessellation);
            int mirrorZCount = 0;
            ...
            int patchCount = TeapotPatches.Length * 2 + mirrorZCount * 2;

            int vertexCount = patchCount * vertexCountPerPatch;
            int indexCount = patchCount * indexCountPerPatch;
